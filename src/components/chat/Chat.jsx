...
function Chat() {
  ...
  return (
    <div className='chat'>
      ...
      <div className="center">
        {chat?.messages?.map((message, index) => (
          <div className={`${message.senderId === currentUser.id ? 'msg own' : 'msg receiver'}`} key={index}>
            <div className="texts-2">
              {message.img && <img src={message.img} className='img-3' alt="..." />}
              <p>{message.text}</p>
            </div>
      ...
      <div className="bottom">
        ...
        <input style={{ marginLeft: '15px' }} type="text" name='text' value={text} placeholder={(isCurrentUserBlocked || isReceiverBlocked) ? "You cannot send message" : 'Type a message...'} onChange={(e) => setText(e.target.value)} disabled={isCurrentUserBlocked || isReceiverBlocked} />
        <div className="emoji">
          <img src="./emoji.png" alt="" onClick={() => setOpen((prev) => !prev)} />
          <div className='picker'><EmojiPicker open={open} onEmojiClick={(e) => handleEmoji(e)} /></div>
        </div>
